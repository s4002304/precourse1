...
def test_insertionsort(dataset_array):
    for a in dataset_array:
        insertionsort(a)


def test_quicksort(dataset_array):
    for a in dataset_array:
        quicksort(a)


def test_quicksort_inplace(dataset_array):
    for a in dataset_array:
        quicksort_inplace(a)
...
def get_random_data():
    return [random.randint(0, 100) for i in range(0, random.randint(0, 50))]
...
if __name__ == "__main__":
    ...
    random_data = []
    for i in range(100):
        random_data.append(get_random_data())
    test_insertionsort(random_data)
    test_quicksort(random_data)
    test_quicksort_inplace(random_data)
